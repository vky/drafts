...
function getAverage(dataArray){
    ...
    // This doesn't look correct, what's the '+' in front of the parens doing?
...
function calculateInterval(sample){
	var n = sample.length;
	var mean = getAverage(sample);
	var sd = getSD(sample);
	var lower = mean - ( DF95[n-1] * (sd/Math.sqrt(n-1)) );
	var upper = mean + ( DF95[n-1] * (sd/Math.sqrt(n-1)) );
	var limits = [mean,lower,upper];
